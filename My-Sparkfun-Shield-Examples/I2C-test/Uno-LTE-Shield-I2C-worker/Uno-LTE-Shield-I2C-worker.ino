...
void setup() {
  ...
  SerialMonitor.println("SerialMonitor connected");
  ...
  if (lte.getOperator(&currentOperator) == LTE_SHIELD_SUCCESS){
    SerialMonitor.println("Current operator: " + currentOperator);
    if (currentOperator == "AT&T"){
      SerialMonitor.println("Current operator is AT&T, skipping 'newConnection'");
      newConnection = false;
    }
  } else {
    SerialMonitor.println("getOperator failed!");
  }
  ...
  if (newConnection) {
    ...
    if (opsAvailable > 0) {
      ...
      int myOp = searchOp("AT&T", ops, opsAvailable);
      if (myOp != -1){
        SerialMonitor.println("Connecting to option " + String(myOp));
        if (lte.registerOperator(ops[myOp]) == LTE_SHIELD_SUCCESS) {
          SerialMonitor.println("Network " + ops[myOp].longOp + " registered\r\n");
        } else {
